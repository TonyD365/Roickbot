...
2. **Start the service** in the app and click **Install MCP config** (writes the `roblox-studio`
   server into your Claude Code config), then restart Claude Code.
...
2. 在应用里**启动服务**并点 **Install MCP config**（把 `roblox-studio` 服务写入 Claude Code 配置），
   然后重启 Claude Code。
